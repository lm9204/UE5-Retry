...
- 하나의 Phase를 한 번에 구현하지 않고 새로운 개념 단위로 나눈다.
- 각 단위 시작 전에 학습 목표, 변경될 게임 동작, 관련 파일, 검증 방법을 설명한다.
- 개념 하나를 구현하고 확인한 뒤 다음 개념으로 넘어간다.
- 서로 독립적인 반복 작업은 병렬 처리할 수 있지만, 사용자가 이해해야 하는 핵심 흐름은 순서를 생략하지 않는다.

### 4. 사용자 실습 기회를 제공한다

- 각 Phase에서 사용자가 직접 수행할 수 있는 작고 대표적인 코드 또는 에디터 작업을 제안한다.
- 에이전트가 수행할 반복 작업, 함께 진행할 핵심 작업, 사용자 실습 후보를 구분한다.
- 사용자가 직접 할지 에이전트가 수행할지 먼저 확인하며, 실습을 강제하지 않는다.
...
- 핵심 코드 변경 전에는 그 코드가 어느 객체에 속하고, 누가 만들며, 언제 사라지고, 누가 호출하는지 설명한다.
- 코드 변경 후에는 파일 목록만 보고하지 말고 입력에서 결과까지의 실행 흐름을 설명한다.
- Phase 종료 시 변경·테스트 결과와 함께 다음을 보고한다.
  - 이번에 사용된 Unreal/C++ 개념
  - 코드에서 개념을 확인할 위치
